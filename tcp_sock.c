...
struct tcp_sock *alloc_tcp_sock()
{
	...
	tsk->iss = tcp_new_iss();
	tsk->snd_una = tsk->snd_nxt = tsk->iss;
	tsk->snd_wnd = tsk->rcv_wnd = TCP_DEFAULT_WINDOW;
...
// 根据端口号对套接字进行绑定，因此子套接字不应加入bind table中
...
// 函数把tsk绑定对应端口，把tsk的端口号作为key放入bind_tsk队列中
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	//wake up, means the connection is established, add to the table
	//？？？？
	//tcp_hash(tsk);
